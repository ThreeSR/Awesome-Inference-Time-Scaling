...
def write_to_readme_in_sorted_order(new_papers, filename="README.md", section_title="## 📖 Paper List (Listed in Time Order)"):
    ...
    Merge new paper entries into the specified section of the README.md,
    sort them by date (newest first), and automatically insert them in the correct position.
    If a paper's title already exists, it will not be added, and "Paper Already Existed!" is printed.
    """
    # Read the entire content of the README.md
    with open(filename, "r", encoding="utf-8") as file:
        content = file.readlines()
    ...
    # Build a set of existing paper titles by extracting the title from each entry
    existing_titles = set()
    ...
    # Filter new papers: if a new paper's title already exists, skip it and print a message
    filtered_new_papers = []
    ...
    # Generate new paper entries in Markdown format from the filtered papers
    new_entries = [format_paper_info(paper) for paper in filtered_new_papers]
